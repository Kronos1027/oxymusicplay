...
private enum class Dest(val route: String, val label: String, val icon: ImageVector) {
    ...
    PLAYER("player", "Player", Icons.Default.Home),
...
fun OxyMusicApp(settings: Settings) {
    ...
    Scaffold(
        bottomBar = {
            NavigationBar(containerColor = colors.background.copy(alpha = 0.95f)) {
        ...
        NavHost(navController = nav, startDestination = Dest.SEARCH.route,
            modifier = Modifier.background(colors.background).padding(padding)) {
            composable(Dest.SEARCH.route) { SearchScreen(onTrackClick = { nav.navigate(Dest.PLAYER.route) }) }
            composable(Dest.PLAYER.route) { PlayerScreen() }
